...
Nuclei = assignParticlesToNuclei (Nuclei,FilledFilteredMask,CompiledParticles);
...
[FractionZeroSpots, FractionOneSpot, FractionOneOrTwoSpots, FractionTwoSpots,...
    FractionTwoOrMoreSpots, FractionActiveNuclei] = NumberOfSpotsFractions (ParticlesPerNucleus);

%P is the probability that a single locus will turn on
% predict P based on the fraction off
p1 = 1-sqrt(FractionZeroSpots);
% predict P based on the fraction with one or more spots
p2 = 1-sqrt(1-FractionOneOrTwoSpots);
% now predict the fraction with two spots, will take the average of the two
% predictions
PredictedTwoSpots = (mean([p1 p2]))^2;
% prediction of one spot
PredictedOneSpot = 2*mean([p1 p2])*(1-mean([p1 p2]));
